...
interface PayWidgetProps {
  request: {
    ...
    note: string | null;
    user: { twitterHandle: string | null; twitterName: string | null; twitterAvatar: string | null; walletAddress: string | null };
  };
}
  ...
  return (
    <div className="hush-card p-5 space-y-4">
      <div className="flex items-center gap-3 p-3 bg-bg rounded-lg border border-line">
        {request.user.twitterAvatar ? (
          <Image src={request.user.twitterAvatar} alt={request.user.twitterName ?? "User"}
        ...
        <div>
          <p className="text-sm font-semibold text-white-2">{request.user.twitterName ?? "Anonymous"}</p>
          {request.user.twitterHandle && <p className="text-xs text-white-4">@{request.user.twitterHandle}</p>}
        </div>
      </div>

      {request.note && <p className="text-sm text-white-3 text-center italic">"{request.note}"</p>}
      ...
      <form onSubmit={async (e) => {
        ...
        const res = await fetch("/api/transfer", {
          ...
          body: JSON.stringify({
            to: request.user.twitterHandle
              ? `@${request.user.twitterHandle}`
