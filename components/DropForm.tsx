...
import { GiftIcon, CopyIcon, CheckIcon } from "@phosphor-icons/react";
...
export function DropForm() {
  ...
  async function handleSubmit() {
    ...
    setUrl(data.url); setAmount(""); setNote("");
  }
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="space-y-3">
        ...
        <div>
          ...
          <input type="text" value={note} onChange={(e) => setNote(e.target.value)}
            placeholder="Enjoy!" maxLength={200} className={inp} />
        </div>
        ...
        <button type="submit" disabled={loading}
          ...
          <GiftIcon size={15} weight="fill" />
          {loading ? "Creating..." : "Create claimable link"}
        </button>
      </form>

      {url && (
        <div className="rounded-lg border border-primary/20 bg-primary-dim p-3">
          <p className="text-xs text-primary font-medium mb-2">Claimable link — share with anyone</p>
